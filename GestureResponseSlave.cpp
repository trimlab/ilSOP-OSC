...
void receiver() 
{
  ...
  while (true) 
  {
    if(DEBUGGING)
    {
      debugFile << "entered receiver loop" << endl;
    }
    ...
    if(DEBUGGING)
    {
      debugFile << "data received: " << itrmdt << endl;
      debugFile.flush();
    }

    splitLine = split(itrmdt, '~');
    if (splitLine.size() == 5) {  // valid input line
...
{
  if (argc < 7) {
    printf("USAGE: GestureResponseSlave left right bottom top num_tracked_objects simulation slave_id\n");
    return 1;
  }
